...
In Figures 3 and 4, the surfaces for the Random-Discrete and Random-Continuous simulations are portrayed. 
...
\subsubsection*{Random-Continuous}
	...
	The non-existent influence of Random-Continuous' parameter is traces back to this fact.


\subsection{Random-Discrete vs. AlwaysSame}
	...
	Having the surface only generated once, results in a different-looking surface.\\
...
\subsection{Random-Discrete vs. Adapt-Discrete}
...
As Adapt-Discrete is defined, its investment is rounded to the nearest integer as expression (\ref{eq:ADPD_rounded}) demonstrates.
		...
		Figure (\ref{fig:surpass_theshold_abs}) exhibits this formula.
		It shows what the opponent's investment has to be at least to surpass the threshold.\\
		\begin{figure}[h]
			\begin{center}
		...
		So to conclude, if Adapt-Discrete's parameter is either 0, 1, or 2, the shift required to overstep the marginal value cannot be achieved.
		Thus, Adapt-Discrete is equivalent to AlwaysCooperate in cases where its parameter is equal to 0, 1, or 2.\\
		This explains the gainfulness of this stripe in the absolute-gain surface of Random-Discrete.
		Adapt-Discrete is exploited since it cannot react and only submits full cooperation in this area.\\
